...
- **Changesets must always use `patch` type** — never `major` or `minor`. This repo is in pre-release mode (`beta` tag) and the version pattern is `0.2.0-beta.*` (this line started at `0.2.0-beta.0` on the `version/0.2.0-beta` branch; `0.1.0-beta.*` is the previous line). In changeset pre-release mode, `major`/`minor` DO bump the actual semver major/minor (e.g. `major` on `0.2.0-beta.16` → `1.0.0-beta.17`), breaking the `0.2.0-beta.*` pattern. Always use `patch` to increment only the beta counter.
...
- **Never add `Co-Authored-By` trailers with AI model names or emails** (e.g. `Co-Authored-By: Claude **** <noreply@anthropic.com>`) to any commit message. Commits are authored solely by the human committer.
- Lint/format toolchain is **package-specific**. Framework packages: **ESLint + Prettier**. `templates/demo-v1`: **Biome**. Don't cross-apply.
