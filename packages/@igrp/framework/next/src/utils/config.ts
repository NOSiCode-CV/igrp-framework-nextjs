...
export function initializeIGRPConfig(customConfig?: Partial<IGRPConfig>): IGRPConfig {
  const defaultConfig: IGRPConfig = {
    appCode: typeof window !== 'undefined' ? (window as any).IGRP_PUBLIC_APP_CODE || 'demo' : 'demo',
    previewMode: typeof window !== 'undefined' ? (window as any).IGRP_PUBLIC_PREVIEW_MODE === 'true' : false,
