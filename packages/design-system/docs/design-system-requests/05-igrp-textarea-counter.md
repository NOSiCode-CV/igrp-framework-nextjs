...
the underlying `Textarea`), so _typing_ past the limit is blocked by the
...
```tsx
<LimitedTextareaField id="fundamentacao" label="Fundamentação" maxLength={500} required />
```
...
(which uses it for both `id` and `name` when `name` is absent) _and_ uses it as
...
The DS has `IGRPColors.soft.warning` but no _semantic_ `--warning` /
...
`warnAt` defaulting to a _fraction_ is a deliberate change from the local
...
import { IGRPTextarea, useIGRPFormContext } from "@igrp/igrp-framework-react-design-system"
...
export function LimitedTextareaField({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="h-0.5 w-full overflow-hidden rounded-full bg-border/60" aria-hidden>
        <div
          className={cn(
      ...
      <div className="flex items-center justify-end">
        <p
          ...
          aria-live="polite"
          className={cn(
            "text-xs tracking-tight tabular-nums transition-colors",
