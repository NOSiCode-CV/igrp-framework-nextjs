 ...
 * every server component importing anything from the package registered the
 * entire design system as its client reference, so every page shipped every
 * component, and server reads of plain values (`IGRP_META_THEME_COLORS`) got a
...
const CLIENT_ONLY_PACKAGES_WITHOUT_DIRECTIVE = ["@origin-space/image-cropper"]
...
describe("design-system client boundaries", () => {
  ...
  it("the root barrel only re-exports", () => {
    ...
    const statements = code
      ...
      .map((s) => s.trim())
      .filter(Boolean)
    const offenders = statements.filter((s) => !/^export\s+(type\s+)?\{[\s\S]*\}\s+from\s+["'][^"']+["']$/.test(s))
  ...
  it.each(serverEvaluated.map((m) => [m.file, m] as const))(
    ...
    (_file, mod) => {
      ...
      for (const pkg of CLIENT_ONLY_PACKAGES_WITHOUT_DIRECTIVE) {
        if (importsValueFrom(code, pkg)) problems.push(`imports ${pkg}, which has no "use client" of its own`)
      }
