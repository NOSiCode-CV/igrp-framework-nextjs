...
const parseAccordionIndex = (value: string) => parseInt(value.replace(ACCORDION_ITEM_PREFIX, ""), 10)
  ...
  return (
    ...
    >
      <div className={cn("flex items-center gap-2")}>
        {showIcon && iconName && (
          <IGRPIcon iconName={iconName} size={iconSize} className={cn("size-4 text-muted-foreground", iconClassName)} />
        )}
        <div>
          <CardTitle className={cn("text-sm font-medium", labelClassName)}>{label}</CardTitle>
          {description && <CardDescription className={cn("text-xs")}>{description}</CardDescription>}
        </div>
      </div>
      {badgeValue && (
...
function FormListFormMode<TItem>({
  ...
  const handleRemove = useCallback(
    (index: number) => {
      ...
      const wasOpen = allowMultipleOpen ? openItems.includes(removedValue) : openItem === removedValue
...
function FormListStandaloneMode<TItem>({
  ...
  return (
    <FormListLayout
      ...
      addDisabled={defaultItem === undefined}
      onItemChangeFactory={
        defaultItem !== undefined ? (index) => (updatedItem: TItem) => handleItemChange(index, updatedItem) : undefined
      }
      ref={ref}
    />
  )
