...
function IGRPStandaloneList<TItem>({
  ...
  useEffect(() => {
    if (!bootstrappedRef.current && value === undefined && (defaultValue === undefined || defaultValue.length === 0)) {
            ...
            >
              <div className={cn("flex justify-between items-center px-4")}>
                ...
                {items.length > 1 && (
                  ...
                  >
                    <IGRPIcon
                      iconName="Trash2"
                      className={cn("h-4 w-4 text-muted-foreground hover:text-destructive hover:bg-destructive/10")}
                      strokeWidth={2}
                    />
                  </Button>
                )}
              ...
              <AccordionContent className={cn("px-4 pb-4")}>
                <StandaloneListItemContent
                  content={renderItem(item ?? defaultItem, index, (updatedItem) =>
