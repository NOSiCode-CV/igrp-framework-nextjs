...
- **NextAuth.js / Auth.js v5** — the `NextAuth({ ... })` factory, `handlers`/`auth`/`signIn`/`signOut` exports, `callbacks.jwt` vs `callbacks.session`, `events`, `trustHost`, session strategies (`jwt` vs `database`), `authorize` for credentials, adapter contracts.
...
- **Next.js 15 middleware** — **edge-runtime constraints** (no Node APIs, limited crypto, no dynamic `require`), cookie read/write patterns, `NextResponse.redirect`/`rewrite`, `matcher` config, interaction with route handlers + Server Components.
...
- **tsup** — `entry` globs, `format: ['esm','cjs']`, `dts`, `treeshake`, externals. Why tsup here (no React Compiler needed for auth logic).
...
- `pnpm build:auth` from repo root. Run `pnpm build:framework` after type changes — `@igrp/framework-next-types` re-exports types from here.
