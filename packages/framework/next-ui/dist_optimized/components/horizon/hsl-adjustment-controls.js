...
function adjustColorByHsl(color, hueShift, saturationScale, lightnessScale) {
  ...
  const out = converter("hsl")(adjustedHsl);
...
const HslAdjustmentControls = () => {
  ...
  const currentHslAdjustments = useMemo(() => themeState.hslAdjustments ?? defaultThemeState.hslAdjustments, [themeState.hslAdjustments]);
  ...
  useEffect(() => {
    debouncedUpdateRef.current = debounce(hslAdjustments => {
      const {
      ...
      const updatedLightStyles = Object.keys(lightStyles).filter(key => !COMMON_STYLES.includes(key)).reduce((acc, key) => {
        const colorKey = key;
        return {
          ...acc,
          [key]: adjustColorByHsl(lightStyles[colorKey], adjustments.hueShift, adjustments.saturationScale, adjustments.lightnessScale)
        };
      }, {});
      const updatedDarkStyles = Object.keys(darkStyles).filter(key => !COMMON_STYLES.includes(key)).reduce((acc, key) => {
        const colorKey = key;
        return {
          ...acc,
          [key]: adjustColorByHsl(darkStyles[colorKey], adjustments.hueShift, adjustments.saturationScale, adjustments.lightnessScale)
        };
      }, {});
  ...
  const handleHslChange = useCallback((property, value) => {
  ...
  const handleBatchHslChange = useCallback(value => {
    if (debouncedUpdateRef.current) {
  ...
  return /*#__PURE__*/_jsxs("div", {
    ...
    children: [/*#__PURE__*/_jsx("div", {
      className: cn("-m-1 mb-2 grid grid-cols-5 gap-2 overflow-hidden p-1 transition-all duration-300 ease-in-out @sm:grid-cols-7 @md:grid-cols-9 @lg:grid-cols-11 @xl:grid-cols-13", !isExpanded ? "h-10" : "h-auto"),
      children: HSL_PRESETS.map(preset => /*#__PURE__*/_jsx(HslPresetButton, {
        label: preset.label,
        ...
        selected: currentHslAdjustments.hueShift === preset.hueShift && currentHslAdjustments.saturationScale === preset.saturationScale && currentHslAdjustments.lightnessScale === preset.lightnessScale,
        adjustColorByHsl: adjustColorByHsl,
        onClick: () => {
          handleBatchHslChange(preset);
        }
      }, preset.label))
    }), HSL_PRESETS.length > 5 && /*#__PURE__*/_jsxs(Button, {
      variant: "ghost",
      size: "sm",
      className: "text-muted-foreground mb-4 flex w-full items-center justify-center text-xs",
      onClick: () => setIsExpanded(!isExpanded),
      children: [isExpanded ? "Hide" : "Show more", " presets", /*#__PURE__*/_jsx(ChevronDown, {
        className: cn("ml-1 h-4 w-4 transition-transform duration-200", isExpanded && "rotate-180")
      })]
    }), /*#__PURE__*/_jsx(SliderWithInput, {
...
//# sourceMappingURL=hsl-adjustment-controls.js.map