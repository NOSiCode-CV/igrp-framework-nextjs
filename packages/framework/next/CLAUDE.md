...
- **Data fetching + caching** — Next 15 default `fetch` cache semantics (no-store by default now), `cache: 'force-cache'`, `next.revalidate`, `next.tags`, `dynamic`/`dynamicIO`, `experimental_ppr` (Partial Prerendering), `unstable_cache`, tag-based invalidation.
- **Streaming + Suspense** — `<Suspense>` boundaries around data-dependent subtrees, `loading.tsx`, `error.tsx`, `not-found.tsx`, interleaving server fallbacks with client hydration.
...
- Public API is `IGRPRootLayout`, `IGRPLayout`, `igrpBuildConfig`, `igrpGetAccessClient`, `igrpGetAccessClientConfig`. Validate breaking changes against `templates/demo/src/app/layout.tsx`, `templates/demo/src/app/(igrp)/layout.tsx`, `templates/demo/src/igrp.template.config.ts`.
