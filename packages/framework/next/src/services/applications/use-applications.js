...
export async function fetchAppByCode(appCode) {
    try {
        if (!appCode)
            throw new Error('Preview Mode is not enabled, when not enabled, Applicattion code is required. Applications Code not found');
...
export async function fetchAppsByUser(username) {
    try {
        if (!username)
