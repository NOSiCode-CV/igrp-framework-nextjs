...
export function FileUploadField(props: FileUploadFieldProps) {
  ...
  const initialFile: FileMetadata | null = useMemo(() => {
    ...
    return value.file instanceof File
      ? { id: value.id, name: value.file.name, size: value.file.size, type: value.file.type, url: value.preview ?? '' }
      : value.file;
  }, [value]);
  ...
  const fileName =
    file?.file instanceof File ? file.file.name : file?.file?.name ?? null;
  ...
  return (
    <IGRPFormItemPrimitive>
        ...
        <div className="flex flex-col gap-2">
          <div className="relative">
            ...
            {previewUrl && !disabled && (
              <div className="absolute top-4 right-4">
                ...
                >
                  <IGRPIcon iconName="X" className="size-4" aria-hidden="true" />
                </button>
              </div>
            )}
          </div>
          ...
          {errors.length > 0 && (
            <div
              ...
              role="alert"
            >
              <IGRPIcon iconName="AlertCircleIcon" className="size-3 shrink-0" />
              <span>{errors[0]}</span>
            </div>
          )}
        </div>
      ...
      {description && (
        <IGRPFormDescriptionPrimitive>{description}</IGRPFormDescriptionPrimitive>
      )}
      <IGRPFormMessagePrimitive />
    </IGRPFormItemPrimitive>
  );
