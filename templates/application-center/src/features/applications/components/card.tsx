...
import { AppWindow, Eye, ExternalLink, Edit } from 'lucide-react';
...
export function ApplicationCard({ app }: { app: Application }) {
  ...
  return (
    <Card className='overflow-hidden card-hover gap-3 pt-4 pb-2 justify-between'>
      ...
      <CardFooter className='flex items-center justify-between px-4'>
        <div className='flex items-center'>
          <Badge className={cn(statusClass(status), 'capitalize')}>{status.toLowerCase()}</Badge>
        </div>
