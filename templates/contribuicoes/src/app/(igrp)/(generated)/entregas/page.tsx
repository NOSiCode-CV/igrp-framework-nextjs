...
export default function PageEntregasComponent() {
  ...
  const [selectPeriodo_ref_fltOptions, setSelectPeriodo_ref_fltOptions] = useState<IGRPOptionsProps[]>([]);
  ...
  function getStatusBadge(value: Declaracao): { iconName?: string, bgClass?: string, textClass?: string, label?: string, className?: string } {
    ...
    <div className={cn('page',)}    >
      <div className={cn('flex', 'flex-col', ' h-full w-full',)}    >
        ...
        <div className={cn('flex', 'flex-1', ' px-6 pb-6 w-full',)}    >
          <IGRPCard
            name={`main_content_card`}

            className={cn('shadow-sm w-full',)}
          >
            <IGRPCardContent
              className={cn('p4 border-b bg-white',)}
            >
              <div className={cn('flex', 'flex-wrap', ' items-center gap-3  px-4',)}    >
                ...
                >
                  <p className={cn()}    >
                    Filtros</p>
                  {hasActiveFilters && (<div className={cn(' ml-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-medium text-primary-foreground',)}    >
                    {(statusFilter !== "" ? 1 : 0) + (periodoReferenciaFilter !== "" ? 1 : 0)}</div>)}
                </IGRPButton>
              ...
              {showFilters && (<div className={cn(' mt-3 pt-3 border-t px-4',)}    >
              ...
              {hasActiveFilters && !showFilters && (<div className={cn(' mt-3 pt-3 border-t flex flex-wrap items-center gap-2 px-4',)}    >
                <p className={cn(' text-xs text-muted-foreground',)}    >
                  Filtros ativos:</p>
                {statusFilter !== "" && (<IGRPBadge
                ...
                >
                  <span className={cn()}    >
                    Estado: {statusFilter === "SUBMETIDA" ? "Submetida" : statusFilter === "VERIFICADA" ? "Verificada" : statusFilter === "PROCESSADA" ? "Processada" : statusFilter === "VALIDADA" ? "Validada" : "ENTREGUE"}</span>
                  <IGRPButton
                ...
                {periodoReferenciaFilter !== "" && (<IGRPBadge
                ...
                >
                  <span className={cn()}    >
                    Mês de Referência: {periodoReferenciaFilter}</span>
                  <IGRPButton
              ...
              <IGRPDataTable<Declaracao, Declaracao>
                ...
                columns={
                  [
                    ...
                    {
                      header: ({ column }) => (<IGRPDataTableHeaderSortToggle column={column} title={`Mês de Referência`} />)
                    ...
                    {
                      header: ({ column }) => (<IGRPDataTableHeaderSortToggle column={column} title={`Total de Remunerações`} />)
                    ...
                    {
                      header: ({ column }) => (<IGRPDataTableHeaderSortToggle column={column} title={`Total de Contribuições`} />)
                                ...
                                [
                                  {
                                    ...
                                    props: {
                                      labelTrigger: `Declarar`, icon: `FilePlus`, iconClassName: `text-blue-500`, showIcon: true,
                                    }
                                  },
                                  {
                                    component: IGRPDataTableDropdownMenuCustom,
                                    props: {
                                      labelTrigger: `Substituir FOS`, icon: `Replace`, iconClassName: `text-amber-500`, showIcon: true,
                                    }
                                  },
                                  {
                                    component: IGRPDataTableDropdownMenuCustom,
                                    props: {
                                      labelTrigger: `Corrigir FOS`, icon: `FileCog`, iconClassName: `text-red-500`, showIcon: true,
                                    }
                                  },
                                  {
                                    component: IGRPDataTableDropdownMenuCustom,
                                    props: {
                                      labelTrigger: `Gerar Cobrança`, icon: `CircleDollarSign`, iconClassName: `text-green-500`, showIcon: true,
                                    }
                                  },
                                ]
                              }
      ...
      <Deliverymodalform isOpen={openDeliveryForm} setIsOpen={() => setOpenDeliveryForm(!openDeliveryForm)} ></Deliverymodalform></div>
  );
}
