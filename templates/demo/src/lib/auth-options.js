...
async function doFinalSignoutHandshake(jwt) {
    ...
    if (idToken && process.env.KEYCLOAK_ISSUER) {
        try {
            ...
            const response = await fetch(`${process.env.KEYCLOAK_ISSUER}/protocol/openid-connect/logout?${params.toString()}`, { method: 'GET' });
        ...
        if (!idToken)
            console.warn('No idToken found for Keycloak post-logout handshake.');
        if (!process.env.KEYCLOAK_ISSUER)
