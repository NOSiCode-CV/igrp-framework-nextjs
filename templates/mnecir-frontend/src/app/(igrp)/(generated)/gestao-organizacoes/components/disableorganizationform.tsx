...
export default function Disableorganizationform({ saveAction, closeModal, id } : { saveAction: (data: object, status: string) => void, closeModal: () => void, id?: string }) {
...
saveAction(payload, form1Data.currentState);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await getCountryAndOrgsById('O', id || '');
 
      if(res) {
        setCurrentState(res.statusDescription);
